/*
  ...
  Nano33BLESensor Library. In this case it outputs barometric pressure data 
  from the Arduino Nano 33 BLE Sense's on board LPS22HB sensor via serial 
  in a format that can be displayed on the Arduino IDE serial plotter. It 
  also outputs the data via BLE in a string format that can be viewed 
  using a variety of BLE scanning software.
...
/* For the use of the pressure sensor */
...
/*GLOBAL Data                                                                */
/*****************************************************************************/
/* Pointer to the Nano33BLEPressure that we will instanciate in the setup */
 ...
 */
BLEService BLEPressure("590d65c7-3a0a-4023-a05a-6aaf2f22441c");
BLECharacteristic pressureBLE("000A", BLERead | BLENotify | BLEBroadcast, BLE_BUFFER_SIZES);
...
void setup()
{
	...
	else
	{
		...
		BLE.setAdvertisedService(BLEPressure);
        /* A seperate characteristic is used for each X, Y, and Z axis. */
		BLEPressure.addCharacteristic(pressureBLE);

		BLE.addService(BLEPressure);
		BLE.advertise();
		/* 
		 * Initialises the pressure sensor, and starts the periodic reading of the 
		 * sensor using a Mbed OS thread. The data is placed in a circular 
		 * buffer and can be read whenever.
		 */
		Pressure = Nano33BLESensor<Nano33BLEPressure>::begin();
...
void loop()
{
	...
	if(central)
	{
		int writeLength;
		/* 
		 * If a BLE device is connected, pressure data will start being read, 
